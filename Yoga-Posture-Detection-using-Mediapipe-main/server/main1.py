...
def compare_pose(image,angle_point,angle_user, angle_target ):
    ...
    overlay = image.copy()
    cv2.rectangle(overlay, (10, 10), (420, 260), (20, 28, 20), -1)
    cv2.addWeighted(overlay, 0.75, image, 0.25, 0, image)
    cv2.rectangle(image, (10, 10), (420, 260), (165, 217, 163), 1) # Subtle border

    height, width, _ = image.shape   
    
    if angle_user[0] < (angle_target[0] - 15):
        stage = stage + 1
        cv2.putText(image, str("Extend the right arm at elbow"), (25,60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, [255, 255, 255], 2, cv2.LINE_AA)
        cv2.circle(image,(int(angle_point[0][0]*width), int(angle_point[0][1]*height)),30,(0,0,255),5) 
        
    if angle_user[0] > (angle_target[0] + 15):
        stage = stage + 1
        cv2.putText(image, str("Fold the right arm at elbow"), (25,90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, [255, 255, 255], 2, cv2.LINE_AA)
        cv2.circle(image,(int(angle_point[0][0]*width), int(angle_point[0][1]*height)),30,(0,0,255),5)
...
with mp_pose.Pose(min_detection_confidence =0.5, min_tracking_confidence = 0.5) as pose:
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if (p_score >= a_score):
                final_score = int((1 - a_score)*100)
            else:
                final_score = int((1 - p_score)*100)

            # --- TIMER LOGIC ---
            h, w, _ = image.shape
            overlay_bottom = image.copy()
            cv2.rectangle(overlay_bottom, (10, h - 140), (450, h - 10), (20, 28, 20), -1)
            cv2.addWeighted(overlay_bottom, 0.75, image, 0.25, 0, image)
            cv2.rectangle(image, (10, h - 140), (450, h - 10), (165, 217, 163), 1)

            if final_score > 90:
                if hold_start_time == 0:
                    hold_start_time = time.time()
                current_hold_time = time.time() - hold_start_time
                cv2.putText(image, f"Holding: {current_hold_time:.1f}s / 5.0s", (30, h - 35), cv2.FONT_HERSHEY_SIMPLEX, 1.0, [0, 255, 0], 2, cv2.LINE_AA)
                if current_hold_time >= 5.0:
                    cv2.putText(image, "SUCCESS!", (image.shape[1]//2 - 150, image.shape[0]//2), cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 255, 0], 5, cv2.LINE_AA)
                    cv2.imshow('MediaPipe Pose', image)
                    cv2.waitKey(3000)
                    break
            else:
                hold_start_time = 0
                cv2.putText(image, "Adjust Pose...", (30, h - 35), cv2.FONT_HERSHEY_SIMPLEX, 1.0, [0, 0, 255], 2, cv2.LINE_AA)

            cv2.putText(image, f"Score: {final_score}%", (30, h - 85), cv2.FONT_HERSHEY_SIMPLEX, 1.0, [0, 0, 255] if final_score < 90 else [0, 255, 0], 2, cv2.LINE_AA)
